...
>
> - There are several followups I have to check before fully disabling. For example TLS certificate verification is globally disabled for all libcurl traffic `src/slic3r/Utils/Http.cpp:170`. This ain't call-home by itself and I assume there is something like https with self-signed cert when logging to some printers... but that should be done per printer if really needed and not disable the VERIFYHOST always.
> - There is still generic cloud/web plumbing that can talk to remote services if someone re-enables or configures it. OrcaCloudServiceAgent defaults to empty base URLs in /src/slic3r/Utils/OrcaCloudServiceAgent.cpp:50, but it will use orca_api_url / orca_auth_url from config if present src/slic3r/Utils/OrcaCloudServiceAgent.cpp:344. sync_user_preset defaults to false in /src/libslic3r/AppConfig.cpp:390, so it is not automatic by default.
> - There is embedded browser in the product and it shows the wikis: Examples are /src/slic3r/GUI/ModelMall.cpp:176, /src/slic3r/GUI/PrinterCloudAuthDialog.cpp:25, /src/slic3r/GUI/PrinterWebView.cpp:72, and the generic loader in
     /src/slic3r/GUI/Widgets/WebView.cpp:349. That’s not hidden behavior, but it means remote web content is still part of the product if those features are used and if those features are used there is an attack surface. I'm considering to replace the links to some more generic ones, to download the texts and images and store it locally when building the binary and by using the system browser to show them.
     Similarly some old Bambu/wiki/tracking URLs remain as data strings and user-click links, for example in /resources/printers/filaments_blacklist.json:45 and multiple GUI wxLaunchDefaultBrowser(...) call sites. I did not find evidence these fire automatically in this fork.
>   - one SVG asset contains an external alicdn.com font reference in /resources/images/ams_readonly.svg:1. I don’t see evidence this is executed as network traffic in normal use, but it is odd and worth cleaning up.
